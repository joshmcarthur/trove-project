...
const invalidStructurePluginPath = join(fixturesDir, "invalid_structure_plugin.ts");
try {
  await Deno.writeTextFile(
...
Deno.test("PluginLoader", async (t) => {
  ...
  await t.step("loads a valid plugin from a factory function export", async () => {
    setup();
  ...
  await t.step("skips plugin file with no default export", async () => {
      ...
      assertStringIncludes(warnLog.message, `Skipping ${invalidExportPluginPath}: no suitable default export found`);
  });
  ...
  await t.step("skips plugin file with invalid structure", async () => {
    ...
    const warnLog = logger.logs.find(l => l.level === 'warn');
    assertExists(warnLog);
    assertStringIncludes(warnLog.message, `Skipping ${invalidStructurePluginPath}: Invalid plugin structure`);
  });
  ...
  await t.step("handles non-existent source path gracefully", async () => {
    ...
    const warnLog = logger.logs.find(l => l.level === 'warn');
    assertExists(warnLog);
    assertStringIncludes(warnLog.message, `Plugin source not found: ${normalize(nonExistentPath)}`);
  });

   await t.step("loads plugins from mixed sources (file and directory)", async () => {
    setup();
