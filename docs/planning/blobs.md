...
- Update share-sheet Shortcut docs to upload photo then POST event with
  `blob_ref`
- `blob_ref` is already accepted by journal and HTTP ingest; blob bytes are stored
  via `PUT /blobs` on http-ingest when `[blobs]` is configured in core
...
- **Blocked by:** none (config loader wired)
