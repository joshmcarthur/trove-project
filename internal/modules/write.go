...
// WriteFromRPC converts req, applies policy when set, and writes the record event.
func (s *WriteService) WriteFromRPC(ctx context.Context, req *troverpc.WriteRequest, policy WritePolicy) (*troverpc.WriteResponse, error) {
	event, err := rpcWriteRequestToJournal(req)
	if err != nil {
		return nil, err
	}
	...
	return writeResultToProto(result), nil
}
...
func writeResultToProto(result WriteResult) *troverpc.WriteResponse {
