...
func TestStartSourceInvokesHealthcheck(t *testing.T) {
	...
	go func() {
		handle, err := StartSource(ctx, store, mod, nil, nil, NewMCPRegistry(), nil, map[string]string{})
...
func TestStartSourceReceivesEmit(t *testing.T) {
	...
	handle, err := StartSource(context.Background(), store, mod, nil, nil, NewMCPRegistry(), nil, map[string]string{})
	if err != nil {
