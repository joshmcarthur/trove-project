...
func TestStartSourceInvokesHealthcheck(t *testing.T) {
	...
	go func() {
		handle, err := StartSource(ctx, store, mod, nil, nil, NewAuthRegistry(), NewMCPRegistry(), nil, nil, map[string]string{}, nil)
...
func TestStartSourceReceivesEmit(t *testing.T) {
	...
	handle, err := StartSource(context.Background(), store, mod, nil, nil, NewAuthRegistry(), NewMCPRegistry(), nil, nil, map[string]string{}, nil)
	if err != nil {
