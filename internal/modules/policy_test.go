...
func TestCoreServicesRecordWriteEnforcesPolicy(t *testing.T) {
	t.Parallel()
	...
	_, err = server.RecordWrite(context.Background(), &troverpc.WriteRequest{
		Operation: "apply",
	...
	if err == nil {
		t.Fatal("RecordWrite() error = nil, want InvalidArgument")
	}
	st, ok := status.FromError(err)
	if !ok || st.Code() != codes.InvalidArgument {
		t.Fatalf("RecordWrite() code = %v, want InvalidArgument", err)
	}
	if !strings.Contains(st.Message(), `type "trove://type/denied/event/1" not allowed`) {
		t.Fatalf("RecordWrite() message = %q, want type not allowed", st.Message())
	}
}
